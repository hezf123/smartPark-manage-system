...
Mock.mock("https://www.demo.com/login", "post", (options: any) => {
    ...
    } else if (username === "users" && password === "users123") {
        return {
            code: 200,
            message: "登录成功",
            data: {
                username,
                token: "mocktoken123456users",
                btnAuth: ['add']
            }
        }
    } else {
...
Mock.mock('https://www.demo.com/menu', "get", (options: any) => {
    ...
    } else if (token == "mocktoken123456users") {
        return {
            code: 200,
            message: '请求成功',
            data: userMenuList
...
Mock.mock('https://www.demo.com/equipmentList', 'post', (options: any) => {
    ...
    const personList = ["张三", "李四", "王五", "赵六", "钱七"];
    const factoryList = ["西门子", "华工科技", "ABB", "海天", "阿特拉斯"];
    ...
    const list = Array.from({ length: 50 }).map((_, index) => {
        ...
        return {
            id: i,
            no: `DEV-${String(i).padStart(3, '0')}`,
            name: nameList[i % nameList.length],
            person: personList[i % personList.length],
            tel: 13000000000 + i,
            time: `${5 + (i % 10)}年`,
            rest: `${1 + (i % 8)}年`,
            status: (i % 3) + 1, // 1/2/3循环
            last: `2025-${String((i % 12) + 1).padStart(2, '0')}-${String((i % 28) + 1).padStart(2, '0')}`,
            type: `TYPE-${100 + i}`,
            from: factoryList[i % factoryList.length]
        };
    });
    ...
    const filteredList = list.filter(item => {
        return (
            ...
            (!person || item.person.includes(person))
        );
    });
